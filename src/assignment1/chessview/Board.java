...
public class Board {
	private @NonNull Piece[][] pieces; // this is the underlying data structure for a board.
	...
	public @NonNull String toString() {
		...
		for(int row=8;row!=0;row--) {
			r += row + "|";
			for(int col=1;col<=8;col++) {
				@Nullable Piece p = pieces[row][col];
				if(p != null) {
					r += p + "|";
				} else {
					r += "_|";
	 ...
	 * @return
	 */
	public boolean clearDiaganolExcept(@NonNull Position startPosition,
			@NonNull Position endPosition, @Nullable Piece... exceptions) {
	 ...
	 * @return
	 */
	public boolean clearColumnExcept(@NonNull Position startPosition,
			@NonNull Position endPosition, @Nullable Piece... exceptions) {
	 ...
	 * @return
	 */
	public boolean clearRowExcept(@NonNull Position startPosition,
			@NonNull Position endPosition, @Nullable Piece... exceptions) {
		int minCol = Math.min(startPosition.column(), endPosition.column());
		int maxCol = Math.max(startPosition.column(), endPosition.column());
		int minRow = Math.min(startPosition.row(), endPosition.row());
	...
	private static boolean contains(@Nullable Piece p1, @Nullable Piece... pieces) {
		if (pieces == null) {
			return false;
		}
		for (Piece p2 : pieces) {
